...
// Exposition HTML
window.startQuiz = startQuiz; window.goToStart = goToStart; window.cancelQuiz = cancelQuiz;
window.showPodium = showPodium; window.openModal = openModal; window.closeModal = closeModal;
window.toggleKeep = toggleKeep; window.resetPodium = resetPodium; window.downloadExcel = downloadExcel;
window.showScreensaver = showScreensaver; window.hideScreensaver = hideScreensaver;
window.goToNextQuestion = goToNextQuestion; window.togglePasswordVisibility = togglePasswordVisibility;
window.generatePlayerPDF = generatePlayerPDF; window.generateAdminPDF = generateAdminPDF;
window.openQuestionEditor = openQuestionEditor; window.closeEditor = closeEditor;
window.addNewQuestion = addNewQuestion; window.editQuestion = editQuestion;
window.saveQuestion = saveQuestion; window.deleteQuestion = deleteQuestion;
window.resetQuestionsToDefault = resetQuestionsToDefault;

// ==========================================
// VARIABLES & CHARGEMENT DES QUESTIONS
// ==========================================
let playerName = ""; let currentQuestions = []; let currentQIndex = 0;
let scoresPoints = {AII: 0, EME: 0, ESE: 0}; let scoresCount = {AII: 0, EME: 0, ESE: 0};
let scoreTotal = 0; let currentStreak = 0; let playerSessionDetails = []; 
let totalQuestions = 30; let timeLimit = 30; let timeLeft = timeLimit;
let timerInterval; let idleTimer; const IDLE_TIME = 120000; 
...
function togglePasswordVisibility() {
    ...
    if (input.type === 'password') { input.type = 'text'; toggleBtn.innerText = '🙈'; } 
    else { input.type = 'password'; toggleBtn.innerText = '👁️'; }
}
...
function setRandomBackground() { document.body.style.backgroundImage = `url('${bgImages[Math.floor(Math.random() * bgImages.length)]}')`; }
...
function playSound(type) {
    ...
    if (type === 'correct') { osc.type = 'sine'; osc.frequency.setValueAtTime(880, now); osc.frequency.setValueAtTime(1046, now + 0.1); gain.gain.setValueAtTime(0.5, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.3); osc.start(now); osc.stop(now + 0.3); } 
    else if (type === 'wrong' || type === 'timeout') { osc.type = 'sawtooth'; osc.frequency.setValueAtTime(150, now); gain.gain.setValueAtTime(0.5, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.4); osc.start(now); osc.stop(now + 0.4); } 
    else if (type === 'fast-tick') { osc.type = 'square'; osc.frequency.setValueAtTime(800, now); gain.gain.setValueAtTime(0.1, now); gain.gain.exponentialRampToValueAtTime(0.001, now + 0.05); osc.start(now); osc.stop(now + 0.05); } 
    else if (type === 'tick') { osc.type = 'square'; osc.frequency.setValueAtTime(400, now); gain.gain.setValueAtTime(0.1, now); gain.gain.exponentialRampToValueAtTime(0.001, now + 0.05); osc.start(now); osc.stop(now + 0.05); } 
    else if (type === 'drumroll') { osc.type = 'sawtooth'; osc.frequency.setValueAtTime(60, now); const lfo = audioCtx.createOscillator(); lfo.type = 'sine'; lfo.frequency.value = 25; const lfoGain = audioCtx.createGain(); lfoGain.gain.value = 50; lfo.connect(lfoGain); lfoGain.connect(osc.frequency); lfo.start(now); lfo.stop(now + 3); gain.gain.setValueAtTime(0, now); gain.gain.linearRampToValueAtTime(0.5, now + 2); gain.gain.linearRampToValueAtTime(0, now + 3); osc.start(now); osc.stop(now + 3); }
}
...
function slideTo(screenId) {
    ...
    if (active) { active.classList.remove('active-screen'); active.classList.add('slide-out-left'); setTimeout(() => { active.classList.add('hidden'); active.classList.remove('slide-out-left'); }, 400); }
    const nextScreen = document.getElementById(screenId); nextScreen.classList.remove('hidden'); nextScreen.classList.add('slide-in-right'); void nextScreen.offsetWidth; nextScreen.classList.remove('slide-in-right'); nextScreen.classList.add('active-screen');
}

function goToStart() {
    document.getElementById('player-name').value = ''; setRandomBackground(); resetIdleTimer(); slideTo('screen-start');
}
...
function cancelQuiz() {
    if (confirm("⚠️ Es-tu sûr de vouloir annuler la partie en cours ?\n\nTa progression ne sera pas sauvegardée et n'apparaîtra pas dans le classement. Tu perdras tout.")) {
        clearInterval(timerInterval); goToStart();
    }
}
function getRandom(arr, n) { let shuffled = [...arr].sort(() => 0.5 - Math.random()); return shuffled.slice(0, n); }
...
async function startQuiz() {
    ...
    let progContainer = document.getElementById('progress-container'); progContainer.innerHTML = '';
...
function loadQuestion() {
    resetIdleTimer(); setRandomBackground(); clearInterval(timerInterval); timeLeft = timeLimit;
    let timerBar = document.getElementById('timer-bar'); timerBar.style.width = "100%"; timerBar.style.backgroundColor = "#2ecc71"; 
    ...
    timerInterval = setInterval(() => {
        ...
        let pct = (timeLeft / timeLimit) * 100; timerBar.style.width = pct + "%";
...
function processAnswer(selectedIndex, correctIndex, clickedBtn) {
    ...
    let allBtns = document.querySelectorAll('.answer-btn'); allBtns.forEach(b => b.disabled = true);
    
    let qData = currentQuestions[currentQIndex];
    let isTimeout = (selectedIndex === -1); let isCorrect = (selectedIndex === correctIndex);
    let box = document.getElementById(`box-${currentQIndex}`); let timeTaken = timeLimit - timeLeft; let pointsGained = 0;
    
    if (isCorrect) {
        scoresCount[qData.cat]++; currentStreak++; playSound('correct');
        if(clickedBtn) clickedBtn.classList.add('btn-correct'); box.classList.add('prog-correct');
        pointsGained = Math.round((timeLeft / timeLimit) * 500) + 500;
        scoreTotal += pointsGained; scoresPoints[qData.cat] += pointsGained;
    } else {
        currentStreak = 0; playSound(isTimeout ? 'timeout' : 'wrong');
        if(clickedBtn) clickedBtn.classList.add('btn-wrong'); box.classList.add('prog-wrong');
...
function showIntermediateScreen(isCorrect, points, trivia, isTimeout) {
    resetIdleTimer();
    let title = document.getElementById('intermediate-title'); let ptsText = document.getElementById('intermediate-points'); let streakText = document.getElementById('intermediate-streak');
    if (isTimeout) { title.innerText = "⏱️ Temps écoulé !"; title.style.color = "#f39c12"; ptsText.innerText = "0 pts"; ptsText.style.color = "#bdc3c7"; } 
    else if (isCorrect) { title.innerText = "Bonne réponse !"; title.style.color = "#2ecc71"; ptsText.innerText = `+ ${points} pts`; ptsText.style.color = "white"; } 
    else { title.innerText = "Aïe, mauvaise réponse..."; title.style.color = "#e74c3c"; ptsText.innerText = "0 pts"; ptsText.style.color = "#bdc3c7"; }
    
    if (currentStreak >= 3) { streakText.innerText = `Série en cours : ${currentStreak} bonnes réponses 🔥 !`; streakText.style.display = "block"; } else { streakText.style.display = "none"; }
    document.getElementById('trivia-text').innerText = trivia; slideTo('screen-intermediate');
}

function goToNextQuestion() {
    currentQIndex++;
    if (currentQIndex < totalQuestions) { slideTo('screen-game'); setTimeout(() => loadQuestion(), 400); } 
    else { slideTo('screen-suspense'); playSound('drumroll'); setTimeout(async () => { await showResults(); window.confetti({ particleCount: 150, spread: 100, origin: { y: 0.6 }, zIndex: 10000 }); }, 3000); }
}
...
function drawRadarChart(canvasId, dataArray, chartInstanceToUpdate) {
    ...
    if (chartInstanceToUpdate) { chartInstanceToUpdate.destroy(); }
    ...
    return new Chart(ctx, {
        ...
        data: {
            labels: ['AII', 'EME', 'ESE'],
            datasets: [{
                label: 'Affinité (%)', data: dataArray,
                backgroundColor: 'rgba(46, 204, 113, 0.4)', borderColor: '#2ecc71',
                pointBackgroundColor: '#f1c40f', pointBorderColor: '#fff',
                pointHoverBackgroundColor: '#fff', pointHoverBorderColor: '#f1c40f',
                borderWidth: 2
            }]
        },
        options: {
...
async function showResults() {
    ...
    try {
        ...
        if (snapshot.exists()) {
            const scoresObj = snapshot.val();
            for (let key in scoresObj) { if (scoresObj[key]["Score Points"] > scoreTotal) { rank++; } }
        }
...
function saveScoreFirebase(name, totalScore, profil, email) {
    push(ref(db, 'scores'), { "Candidat": name, "Score Points": totalScore, "Profil": profil, "ScoresCount": scoresCount, "ScoresPoints": scoresPoints, "SessionDetails": playerSessionDetails, "keep": false, "Email": email || "" });
}
...
async function showPodium() {
    resetIdleTimer(); slideTo('screen-podium'); let tbody = document.getElementById('podium-body'); tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#bdc3c7;">Chargement <div class="spinner"></div></td></tr>';
    try {
        const snapshot = await get(ref(db, 'scores')); let data = [];
        if (snapshot.exists()) { const scoresObj = snapshot.val(); for (let key in scoresObj) { data.push({ id: key, ...scoresObj[key] }); } }
        data.sort((a, b) => b["Score Points"] - a["Score Points"]); tbody.innerHTML = '';
        if(data.length === 0) { tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#bdc3c7;">Aucun joueur enregistré.</td></tr>'; } 
        else {
            data.forEach((joueur, index) => {
                let medaille = index === 0 ? "🥇" : (index === 1 ? "🥈" : (index === 2 ? "🥉" : ""));
                tbody.innerHTML += `<tr><td>${medaille} ${index+1}</td><td>${joueur.Candidat}</td><td>${joueur["Score Points"]}</td><td>${joueur.Profil}</td><td><button class="btn-details" onclick="openModal('${joueur.id}')">Détails 🔍</button></td></tr>`;
            });
        }
    } catch (error) { console.error(error); tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#e74c3c;">Erreur réseau ❌</td></tr>'; }
}

async function openModal(playerId) {
    try {
        const snapshot = await get(ref(db, 'scores')); if(!snapshot.exists()) return;
        ...
        allPlayers.forEach(p => {
            if(p.SessionDetails) { p.SessionDetails.forEach(q => { if(!globalStats[q.q]) globalStats[q.q] = { asked: 0, correct: 0 }; globalStats[q.q].asked++; if(q.isCorrect) globalStats[q.q].correct++; }); }
        });
        ...
        if (player.SessionDetails) {
            player.SessionDetails.forEach(q => {
                ...
                tbody.innerHTML += `<tr><td style="text-align:center;">${q.cat}</td><td style="text-align:center;">${q.diff || '-'}</td><td>${q.q}</td><td style="text-align:center;">${resIcon}</td><td style="text-align:center;">${q.points}</td><td style="text-align:center;">${successRate}</td></tr>`;
            });
        }
...
async function toggleKeep(playerId, isChecked) { try { await set(ref(db, 'scores/' + playerId + '/keep'), isChecked); } catch (e) { console.error(e); } }
async function resetPodium() {
    ...
    if (pwd === ADMIN_PASSWORD) {
        if (confirm("⚠️ Voulez-vous vraiment effacer TOUS les scores ? Action irréversible.")) {
            try { await set(ref(db, 'scores'), null); alert("🗑️ Base de données réinitialisée !"); showPodium(); } catch (error) { alert("Erreur."); }
        }
...
async function downloadExcel() {
    ...
    if (pwd === ADMIN_PASSWORD) {
        try {
            const snapshot = await get(ref(db, 'scores')); if (!snapshot.exists()) return alert("Aucune donnée.");
            let dataPlayers = []; let dataQuestions = []; let dataDetails = [];
            const scoresObj = snapshot.val(); let allPlayers = Object.values(scoresObj);
            
            for (let key in scoresObj) {
                let p = scoresObj[key];
                dataPlayers.push({ "Candidat": p.Candidat || "Inconnu", "Score Global": p["Score Points"] || 0, "Profil": p.Profil || "", "Points AII": p.ScoresPoints ? p.ScoresPoints.AII : 0, "Points EME": p.ScoresPoints ? p.ScoresPoints.EME : 0, "Points ESE": p.ScoresPoints ? p.ScoresPoints.ESE : 0, "Bonnes Rép. AII": p.ScoresCount ? p.ScoresCount.AII : 0, "Bonnes Rép. EME": p.ScoresCount ? p.ScoresCount.EME : 0, "Bonnes Rép. ESE": p.ScoresCount ? p.ScoresCount.ESE : 0, "Email Contact": p.Email || "", "Conserver": p.keep ? "OUI" : "NON" });
                if (p.SessionDetails) { p.SessionDetails.forEach((q, index) => { dataDetails.push({ "Candidat": p.Candidat, "Num": index + 1, "Catégorie": q.cat, "Diff": q.diff || '-', "Question": q.q, "Résultat": q.isCorrect ? "VRAI" : "FAUX", "Temps (s)": q.time, "Points": q.points }); }); }
            }
            dataPlayers.sort((a, b) => b["Score Global"] - a["Score Global"]);
            
            let globalStats = {};
            allPlayers.forEach(p => {
                if (p.SessionDetails) { p.SessionDetails.forEach(q => { if (!globalStats[q.q]) { globalStats[q.q] = { cat: q.cat, asked: 0, correct: 0 }; } globalStats[q.q].asked++; if (q.isCorrect) globalStats[q.q].correct++; }); }
            });
            for (let qText in globalStats) {
                let stat = globalStats[qText]; let successRate = stat.asked > 0 ? Math.round((stat.correct / stat.asked) * 100) + "%" : "0%";
                dataQuestions.push({ "Catégorie": stat.cat, "Question": qText, "Fois Posée": stat.asked, "Bonnes Réponses": stat.correct, "Taux Réussite": successRate });
            }
...
function editQuestion(index) {
    ...
    let formHtml = `
        ...
        <select id="edit-cat" class="editor-select"><option value="AII" ${q.cat==='AII'?'selected':''}>AII</option><option value="EME" ${q.cat==='EME'?'selected':''}>EME</option><option value="ESE" ${q.cat==='ESE'?'selected':''}>ESE</option></select>
        <select id="edit-diff" class="editor-select"><option value="Com" ${q.diff==='Com'?'selected':''}>Com (1x)</option><option value="STI" ${q.diff==='STI'?'selected':''}>STI (2x)</option><option value="BU1" ${q.diff==='BU1'?'selected':''}>BU1 (3x)</option><option value="BU2" ${q.diff==='BU2'?'selected':''}>BU2 (4x)</option></select>
        <br><input type="text" id="edit-q" class="editor-input" value="${q.q.replace(/"/g, '&quot;')}" placeholder="Texte de la question">
        ...
        <button class="main-btn" style="width:auto; padding:10px 20px; font-size:1.1em;" onclick="saveQuestion(${index})">💾 Sauvegarder dans la base</button>
        <button class="secondary-btn" style="background:#e74c3c; padding:10px 20px; font-size:1.1em;" onclick="document.getElementById('editor-form-container').classList.add('hidden')">Annuler</button>
    `;
...
async function saveQuestion(index) {
    ...
    let diff = document.getElementById('edit-diff').value;
    let textQ = document.getElementById('edit-q').value;
    let o0 = document.getElementById('edit-o0').value;
    let o1 = document.getElementById('edit-o1').value;
    let o2 = document.getElementById('edit-o2').value;
    let o3 = document.getElementById('edit-o3').value;
    let trivia = document.getElementById('edit-trivia').value;
    
    let ansRadios = document.getElementsByName('edit-ans');
    let ans = 0;
    for(let i=0; i<ansRadios.length; i++) { if(ansRadios[i].checked) ans = parseInt(ansRadios[i].value); }

    dynamicDB[index] = { cat: cat, diff: diff, q: textQ, opt: [o0, o1, o2, o3], ans: ans, trivia: trivia };
...
// ==========================================
function resetIdleTimer() { clearTimeout(idleTimer); idleTimer = setTimeout(showScreensaver, IDLE_TIME); }
document.addEventListener('mousemove', resetIdleTimer); document.addEventListener('click', resetIdleTimer); document.addEventListener('touchstart', resetIdleTimer);

async function showScreensaver() {
    const screen = document.getElementById('screensaver'); screen.classList.remove('hidden');
    let scrollContent = document.getElementById('screensaver-scroll'); scrollContent.innerHTML = 'Chargement du classement...';
    try {
        const snapshot = await get(ref(db, 'scores'));
        if (snapshot.exists()) {
            let data = Object.values(snapshot.val()); data.sort((a, b) => b["Score Points"] - a["Score Points"]);
            let top10 = data.slice(0, 10); let html = "";
            top10.forEach((p, i) => { let medaille = i === 0 ? "🥇" : (i === 1 ? "🥈" : (i === 2 ? "🥉" : `${i+1}.`)); html += `<div>${medaille} ${p.Candidat} - ${p["Score Points"]} pts</div>`; });
...
function hideScreensaver() { document.getElementById('screensaver').classList.add('hidden'); resetIdleTimer(); }
resetIdleTimer();
...
document.addEventListener('keydown', function(e) {
    ...
    // Réveille l'écran (N'importe quelle touche)
    if (!screensaver.classList.contains('hidden')) { hideScreensaver(); return; }

    // ECHAP (Fermer)
    if (e.key === 'Escape') {
        if (pwdModal.classList.contains('show')) { /* géré en local */ }
        else if (detailsModal.classList.contains('show')) { closeModal(); }
    ...
    // ENTREE (Validation contextuelle sans focus forcé)
    if (e.key === 'Enter') {
        if (activeScreen) {
    ...
    // FLECHES (Sélection des réponses de la question)
    if (activeScreen && activeScreen.id === 'screen-game') {
        if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
            ...
            if (currentIndex === -1) { btns[0].focus(); } 
            else {
